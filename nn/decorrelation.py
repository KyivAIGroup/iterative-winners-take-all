...
w_xy, w_xh, w_hy, w_hh, w_yy, w_yh = {}, {}, {}, {}, {}, {}
for mode in stats.keys():
    if WITH_PERMANENCE:
        w_xy[mode] = ParameterWithPermanence(torch.rand(N_x, N_y), sparsity=s_w_xy)
        w_xh[mode] = ParameterWithPermanence(torch.rand(N_x, N_h), sparsity=s_w_xh)
        w_hy[mode] = ParameterWithPermanence(torch.rand(N_h, N_y), sparsity=s_w_hy)
        w_hh[mode] = ParameterWithPermanence(torch.rand(N_h, N_h), sparsity=s_w_hh)
        w_yh[mode] = ParameterWithPermanence(torch.rand(N_y, N_h), sparsity=s_w_yh)
        w_yy[mode] = ParameterWithPermanence(torch.rand(N_y, N_y), sparsity=s_w_yy)
    else:
...
iwta = IterativeWTA(w_xy=w_xy['iWTA'], w_xh=w_xh['iWTA'], w_hy=w_hy['iWTA'], w_hh=w_hh['iWTA'], w_yy=w_yy['iWTA'], w_yh=w_yh['iWTA'])
