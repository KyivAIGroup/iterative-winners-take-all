...
s_w_xy = 0.1
s_w_xh = 0.1
w_xy = s_w_xy * np.random.rand(N_y, N_x)
w_xh = s_w_xh * np.random.rand(N_h, N_x)
...
s_w_yh = 0.01
s_w_yy = 0.01
w_hh = s_w_hh * np.random.rand(N_h, N_h) #np.random.binomial(1, s_w_hh, size=(N_h, N_h))
w_hy = s_w_hy * np.random.rand(N_y, N_y) #np.random.binomial(1, s_w_hy, size=(N_y, N_h))
...
def activate(x, learning=False):
    ...
    t_stop = np.max([np.max(h_o), np.max(y_o)])
    t_range = np.linspace(0, t_stop, 50)[::-1]
    # print(t_range)
    for ti in t_range:
        z_h = h_o - w_hh @ h >= ti
        z_y = y_o - w_hy @ h >= ti
