...
# compare cos similarity to simple kWTA

# if kwta uses the same sparsity, the cosine similarity the same (very close)
...
for i in range(iters):
    ...
    data[i] = cos(x1, x2), cos(y1, y2), cos(h1, h2)
    data_kwta[i] = cos(x1, x2), cos(y_kwta1, y_kwta2)
    sparsity[i] = np.count_nonzero(y1) / N_y, np.count_nonzero(h1) / N_h
