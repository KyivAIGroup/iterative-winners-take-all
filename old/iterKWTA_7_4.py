...
# check how similarity preservation changes with learning

# learning h->y connections (increase) reduces sparsity and cos similarity. not clear how the decrease relates to kwta

# learning y->y increases similarity
import numpy as np
...
print(np.count_nonzero(y1) / N_y, np.count_nonzero(y2) / N_y)
print('similarity', cos(x1, x2), cos(y1, y2))
...
for i in range(iters):
    # x1 = np.random.binomial(1, s_x, size=N_x)
    # x2 = np.random.binomial(1, s_x, size=N_x)
    y1, h1 = activate(x1, learning=True)
    y2, h2 = activate(x2, learning=True)
...
s_y1 = np.count_nonzero(y1) / N_y
s_y2 = np.count_nonzero(y2) / N_y
print('sparsity',  s_y1, s_y2)


y_kwta1 = kWTA(w_xy @ x1, s_y1)
y_kwta2 = kWTA(w_xy @ x2, s_y2)

print('similarity',  cos(y1, y2))
print('similarity kwta',  cos(y_kwta1, y_kwta2))
print(y1@y_kwta1.T, np.count_nonzero(y1), np.count_nonzero(y_kwta1))
print(y2@y_kwta2.T, np.count_nonzero(y2), np.count_nonzero(y_kwta2))
# y_kwta2 = kWTA(w_xy @ x2, s_y2)
