...
for repeat in trange(N_REPEATS):
    ...
    for epoch in range(N_ITERS):
        ...
        for y0, h0 in zip(y0_batch.T, h0_batch.T):
            h, y = iWTA(y0=y0, h0=h0, w_hy=w_hy, w_hh=w_hh)
            update_weights(w_hy, x_pre=h, x_post=y, n_choose=NUM_TO_LEARN)

        y_kwta_pre = y0_batch - w_hy @ h0_batch
        _, y_batch = iWTA(y0=y0_batch, h0=h0_batch, w_hy=w_hy, w_hh=w_hh)
        y_kwta_fixed_k = kWTA(y_kwta_pre, k=K_FIXED)
        n_active_batch = np.count_nonzero(y_batch, axis=0)
        y_kwta = kWTA_different_k(y_kwta_pre,
                                  ks=n_active_batch)
        n_active[repeat, epoch] = n_active_batch.mean()

        stats['iWTA'][repeat, epoch] = compute_discriminative_factor(y_batch, labels)
        stats['kWTA'][repeat, epoch] = compute_discriminative_factor(y_kwta, labels)
        stats['kWTA-fixed-k'][repeat, epoch] = compute_discriminative_factor(y_kwta_fixed_k, labels)
